...
int main()
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load(RESOURCE_DIRECTORY "textures/logo2.png", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while(!glfwWindowShouldClose(Window))
    {
        ...
        // render boxes
        glBindVertexArray(VAO);
        auto time = (float)glfwGetTime() / 2;
        for (unsigned int i = 0; i < SQUARES_TO_DRAW; i++) {
            // calculate the model matrix for each object and pass it to shader before drawing
            glm::mat4 model = glm::mat4(1.0f);
            model = glm::translate(model, cubePositions[i]);
            float angle = 20.0f * static_cast<float>(i);
            model = glm::rotate(model, glm::radians(angle) + time, glm::vec3(1.0f, 1.0f, 0.0f));
            model = glm::scale(model, glm::vec3(1.0f + 0.5*sin(time), 1.0f + 0.5*sin(time), 1.0f + 0.5*sin(time)));
