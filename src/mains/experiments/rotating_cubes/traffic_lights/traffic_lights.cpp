...
int main()
{
    ...
    unsigned char *data = stbi_load(RESOURCE_DIRECTORY "textures/ed_YellowBorder.png", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load(RESOURCE_DIRECTORY "textures/Yellow_Green-Border.png", &width, &height, &nrChannels, 0);
    if (data)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load(RESOURCE_DIRECTORY "textures/Green_Red-Border.png", &width, &height, &nrChannels, 0);
    if (data)
    {
